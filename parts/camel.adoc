...
.bean([highlight]#MyBean.class#); // lookup by type
...
. Detect by observing the `ProcessAnnotatedType` lifecycle events:
.. The beans of type `RouteBuilder` to instantiate and add them to the Camel context
.. The beans containing Camel annotations
. Change how CDI beans containing Camel annotations get injected by observing the `ProcessInjectionTarget` lifecycle event
...
icon:question-circle[] How the detect a bean exists in the deployment?
...
public class CamelExtension implements Extension {
    void addContextBean(@Observes [highlight]#AfterBeanDiscovery# abd, BeanManager manager) {
        Set<Bean<?>> beans = manager.[highlight]#getBeans#(CamelContext.class);
        if (beans.isEmpty())
...
TIP: Then add the `CamelContextBean` bean programmatically
...
public class CamelExtension implements Extension {
    ...
    void addContextBean(@Observes [highlight]#AfterBeanDiscovery# abd, BeanManager manager) {
        Set<Bean<?>> beans = manager.getBeans(CamelContext.class);
        if (beans.isEmpty())
...
public class CamelExtension implements Extension {
    ...
    void configureContext(@Observes [highlight]#AfterDeploymentValidation# adv, BeanManager bm) {
        ...
        for (Bean<?> bean : bm.getBeans(RoutesBuilder.class)
            context.[highlight]#addRoutes#(getReference(bm, RouteBuilder.class);
    }
...
public class CamelExtension implements Extension {
    ...
    void configureContext(@Observes [highlight]#AfterDeploymentValidation# adv, BeanManager bm) {
        CamelContext context = getReference(bm, CamelContext.class);
        for (Bean<?> bean : bm.getBeans(RoutesBuilder.class)
            context.addRoutes(getReference(bm, RouteBuilder.class);
...
icon:question-circle[] How to reference CDI beans in the Camel DSL?
...
class CdiRegistry implements Registry {
    ...
    public <T> T lookupByNameAndType(String name, Class<T> type) {
        return getReference(bm, type, bm.getBean(name));
    }
    public <T> Set<T> findByType(Class<T> type) {
        return getReference(bm, type, bm.resolve(getBeans(type));
    }
...
[.statement]
====
TIP: We can get rid of all the code related to the Camel context config
...
icon:question-circle[] How to support custom injection annotations?
