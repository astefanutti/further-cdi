...
public class CamelExtension implements Extension {
    ...
    void addCamelContextBean(_@Observes_ [highlight]_AfterBeanDiscovery_ abd) {
        abd.<CamelContext>[highlight]##addBean()##
...
public class MetricsExtension implements Extension {
  ...
  <T> void camelBeansPostProcessor(_@Observes_ _ProcessInjectionTarget<T>_ pit, _BeanManager_ manager) {
  ...
  void configureCamelContext(_@Observes_ _AfterDeploymentValidation_ adv, _BeanManager_ manager) {
    ...
    context.addInterceptStrategy((camel, definition, target, next) -> definition.hasCustomIdAssigned()
      ? nodes.stream().filter(node -> definition.getId().equals(node.value())).findFirst()
        .map(node -> (Processor) new DelegateAsyncProcessor(target) {
          public boolean process(Exchange exchange, AsyncCallback callback) {
