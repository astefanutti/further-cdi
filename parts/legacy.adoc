...
class FpmlToMxmlTransformationMain throws Exception {
    public static void main(String[] args) {
        ...
        ssdParameters.setObject(inputFpml);
...
. Wrap the legacy component as a CDI bean programmatically by implementing the `Bean` SPI and observing the `AfterBeanDiscovery` lifecycle event
. Declare a producer method that will produce the `Transformer` facade instances
...
TIP: We first need to collect all the `@Formula` pathes metadata required to instantiate the legacy component
...
public interface AfterBeanDiscovery {
    public void addDefinitionError(Throwable t);
    [highlight]#public void addBean(Bean<?> bean);#
...
SDDParameterProvider parameters = ParameterUtils.buildParameters(new Properties());
