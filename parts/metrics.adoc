...
class MetricProducer<X extends Metric> implements Producer<X> {
    ...
    MetricProducer(BeanManager manager, Producer<X> decorate, String metricName) {
...
----
    public X produce(CreationalContext<X> ctx) { <1>
        ...
        if (!reg.getMetrics().containsKey(metricName)) <2>
...
<2> If metric name is not in the registry, the original producer is called and its result is added to the registry
...
public class MetricsExtension implements Extension {
    ...
    <X extends Metric> void decorateMetricProducer(_@Observes_ [highlight]_ProcessProducer<?, X>_ pp,
       ...
       if (pp.getAnnotatedMember().isAnnotationPresent(Metric.class)) {
            ...
            [highlight]_pp.setProducer_(new [highlight]#MetricProducer<>#(manager, [highlight]_pp.getProducer()_, name)); <2>
        }
    }
    // ...
}
...
public class MetricsExtension implements Extension {
  // ...
  void registerProducedMetrics(_@Observes_ [highlight]_AfterDeploymentValidation_ adv, BeanManager manager) {
        manager.createInstance().select(com.codahale.metrics.Metric.class)
