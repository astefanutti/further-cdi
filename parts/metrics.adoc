...
class MetricRegistryBean {
    [highlight]#@Produces @ApplicationScoped#
...
CAUTION: Apply metrics with provided annotations


CAUTION: Access same *Metric* through inject or `MetricRegistry` getter
...
WARNING: This code create a deployment error (ambiguous dependency)
...
----
    ...
    *@Produces*
    Timer timer2 = new Timer(new SlidingTimeWindowReservoir(1L, HOURS)); <2>
    ...
    *@Inject*
    Timer timer; <3>
----
...
----
    ...
    *@Produces*
    ...
    Timer timer2 = new Timer(new SlidingTimeWindowReservoir(1L,HOURS));
    ...
    *@Inject*
    [highlight]*@Metric(name="myTimer")*
    Timer timer;
...
TIP: A CDI extension is a class implementing empty `Extension` interface
...
TIP: And have the `Timer` injection point satisfied
...
CAUTION: The solution is to build and interceptor and bind it to `@Timed`
...
CAUTION: Interceptor should be enhanced to support `@Timed` on a class

CAUTION: Other interceptor shoudl be developped for other metrics
====
...
@Inject
@Metric(name = "myTimer")
[highlight]*Timer* timer;

public void aMethod() {
    Timer [highlight]*myTimer* = registry.timer("myTimer");
    }
----

TIP: ... we want that `timer == myTimer`
...
public void aMethod() {
    Timer [highlight]*myTimer* = registry.timer("myTimer"); <3>
}
----
<1> Produced `Timer` should be added to MetricRegistry when produced
<3> When retrieved from regisitry a `Metric` should be *identical* to the produced instance and vice versa
...
----
    ...
    public Set<InjectionPoint> getInjectionPoints() { return decorate.getInjectionPoints(); }
}
----
...
<3> If metric name is not in registry original producer is called and its result is added to registry
...
NOTE: This event allow us to substitute the original producer by ours
...
----
<1> we retrieve Metric name from `name()` member in `@Metric`
...
public class MetricsExtension implements Extension {
    //...
    void registerProducedMetrics([highlight]*@Observes*@Observes AfterDeploymentValidation adv) {
        List<Metric> metrics = BeanProvider.getContextualReferences(Metric.class,true); <1>
    }
    //...
}
----
<1> `getContextualReferences()` is a method in `BeanProvider` *DeltaSpike* helper class. +
It creates the list of bean instances for a given bean type (ignoring qualifier)
====
