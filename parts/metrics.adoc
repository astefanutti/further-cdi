...
[.topic.source]
== Without a CDI Extension

[source, subs="verbatim,quotes"]
----
class MetricRegistryBean {
    @Produces @ApplicationScoped MetricRegistry registry = new MetricRegistry();
}
----
...
TIP: Use the CDI SPI to add the interceptor binding in a CDI extension programmatically
...
[.topic.source]
== Declare Interceptor Bindings Directly

TIP: Use the CDI SPI to directly declare the `@Timed` annotation as interceptor binding

[source, subs="verbatim,quotes"]
----
class MetricsExtension implements Extension {

  <X> void addInterceptorBinding(@Observes [highlight]#BeforeBeanDiscovery# bbd) {
    AnnotatedType<Timed> annotated = manager.createAnnotatedType(Timed.class);
    Set<AnnotatedMethod<? super Timed>> methods = new HashSet<>();
    for (AnnotatedMethod<? super Timed> method : annotated.getMethods())
        methods.add(new AnnotatedMethodDecorator<>(method, new NonbindingLiteral()));

    bbd.addInterceptorBinding(new AnnotatedTypeDecorator<>(annotated, methods));
  }
}
----
...
TIP: Use *producer fields / methods* to declare custom metrics
...
TIP: Use the producer fields and methods to instantiate and register the metrics at deployment time
...
[.topic.source]
== `ProcessProducer` in Lifecycle

[plantuml, "lifecycle-pp", "svg", height="85%", width="95%"]
----
@startuml
